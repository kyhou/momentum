<template>
    <Page>
        ...
        <label for="modal-profit" class="btn btn-primary" @click="openAddModal">Criar Vigência</label>

        <!-- Put this part before </body> tag -->
        <input type="checkbox" id="modal-profit" class="modal-toggle" />
        <div class="modal">
            <div class="modal-box relative">
                <label for="modal-profit" class="btn btn-circle btn-sm absolute right-2 top-2">✕</label>
                <section class="mx-auto max-w-4xl rounded-md bg-white p-6 shadow-md dark:bg-primary">
                    <h2 class="text-lg font-semibold capitalize text-gray-700 dark:text-white">
                        {{ this.editMode ? "Alterar Vigência" : "Nova Vigência" }}
                    </h2>
                    <Form id="addForm" ref="addForm" @submit="handleAdd" :validation-schema="schema">
                        <div v-if="!successful">
                            <div class="form-group mt-4 gap-6">
                                ...
                                <div>
                                    <label class="text-gray-700 dark:text-gray-200" for="startDate">Data Inicial</label>
                                    <Field id="startDate" name="startDate" type="date" ref="startDate"
                                ...
                                <div>
                                    <label class="text-gray-700 dark:text-gray-200" for="endDate">Data Final</label>
                                    <Field id="endDate" name="endDate" type="date" ref="endDate"
                                        class="focus:ring-opacity/40 form-control mt-2 block w-full rounded-md 
                                                                border border-gray-200 bg-white px-4 py-2 text-gray-700 
                                                                focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 
                                                                dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:focus:border-blue-300" />
...
import ProfitsAdminPageService from "../services/profits_admin_page.service";
...
export default {
    ...
    data() {
        const schema = yup.object().shape({
            ...
            aporte: yup.string().required("Favor selecionar o aporte"),
            pct: yup.number().typeError("Favor informar o valor válido para a % de rendimento").required("Favor informar o valor para a % de rendimento"),
            startDate: yup.date().typeError("Favor informar uma data válida").required("Favor selecionar uma data")
        });
    ...
    mounted() {
        ProfitsAdminPageService.getProfits().then(
            response => {
    ...
    methods: {
            ...
            grid.getPlugins().then(plugins => {
                plugins.forEach(p => {
                    if (p.exportFile) {
                        ...
                        exportPlugin.exportFile({ filename: 'Vigencias-' + moment().format("DD/MM/YYYY") });
                    }
                })
            });
        },
        handleAdd(profit) {
            ...
            if (!profit.editMode) {
                ProfitsAdminPageService.newProfit(profit).then(
                    data => {
                        this.message = data.message;
                        this.successful = true;
                        this.loading = false;

                        ProfitsAdminPageService.getProfits().then(response => {
                            this.rows = response.data;
                        });
                    },
                    error => {
                ...
                ProfitsAdminPageService.updateProfit(profit).then(
                    data => {
        ...
        showEditModal(data) {
            ...
            ProfitsAdminPageService.getUserAportes(data.model.userId).then(response => {
                ...
                this.$refs.addForm.setValues({
                    ...
                    pct: data.model.value,
                    startDate: moment(data.model.startDate, "DD/MM/YYYY").format("YYYY-MM-DD"),
                    endDate: moment(data.model.endDate, "DD/MM/YYYY").format("YYYY-MM-DD"),
                    editMode: true
                });
            }).catch(() => { });
        },
        ...
        onChange(event) {
            ProfitsAdminPageService.getUserAportes(event.target.value).then(response => {
                this.aportes = response.data.aportes;
            }).catch(() => { });
        },
